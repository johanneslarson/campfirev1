...
const markers = [
  ...
  { name: "Portland", coordinates: [-122.7, 45.5] as [number, number] },
  { name: "Boston", coordinates: [-71.1, 42.3] as [number, number] }
];
...
const MusicMap: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Fetch artist details from the getAllArtists API
    const loadArtistDetails = async () => {
      try {
        // Get all artists from the API (this will use the cache if already loaded)
        ...
        // Map selected community artists to their full details
        const details = selectedCommunity.artists.map(artist => {
          const fullArtist = artistMap[artist.id];
          if (!fullArtist) return { id: artist.id, name: artist.name };
          
          return {
            id: artist.id,
            name: artist.name,
            bio: fullArtist.bio,
            imageUrl: fullArtist.imageUrl || `/assets/artists/${artist.name.replace(/ /g, '')}.jpg`
          };
        });
        ...
        const basicDetails = selectedCommunity.artists.map(artist => ({
          id: artist.id,
          name: artist.name
        }));
        setArtistDetails(basicDetails);
        setShowPopup(true);
      }
    };
