...
import { getFeaturedTracks, getPlatformStats, getCommunityStories, Track, getIsInitialized, getAllArtistsSync, Artist } from "../services/data";
import { FaIcons } from "../utils/icons";

// Brand colors for gradients
// Removed BRAND_COLORS as they are now handled by Tailwind config

// Featured artists to display (MadFrances, SYM1, and Kiyan Saifi)
const FEATURED_ARTIST_NAMES = ["MadFrances", "SYM1", "Kiyan Saifi"];
...
function Home() {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-screen-xl mx-auto space-y-6 sm:space-y-8">
      ...
      <section>
        <h2 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-accent">Featured Artists</h2>
              ...
              >
                ...
                {artist.imageUrl ? (
                  ...
                  <div className="w-full h-full bg-dark-lighter flex items-center justify-center">
                    <FaIcons.FaMusic size={48} className="text-primaryLight" />
                  </div>
                )}
                ...
                {/* Hover Effect */}
                <div className="absolute inset-0 bg-primaryLight/20 opacity-0 group-hover:opacity-100 transition-opacity"></div>
              </Link>
            ))}
          </div>
        )}
