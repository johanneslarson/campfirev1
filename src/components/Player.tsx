...
function Player() {
  ...
  useEffect(() => {
    ...
    if (currentTrack) {
      ...
      console.log("[Player.tsx] Current track changed or isPlaying changed. Current track:", currentTrack.title, "URL:", currentTrack.url, "Is Playing:", isPlaying);
      
      // Only update the source if it's different from the current one
      if (audioRef.current.src !== currentTrack.url) {
        console.log("[Player.tsx] Setting audio source to:", currentTrack.url);
        audioRef.current.src = currentTrack.url;
        audioRef.current.load(); // Explicitly call load
      }
      
      if (isPlaying) {
        console.log("[Player.tsx] Attempting to play audio. Current src:", audioRef.current.src);
        const playPromise = audioRef.current.play();
        if (playPromise !== undefined) {
          playPromise.then(_ => {
            console.log("[Player.tsx] Audio playback started successfully for:", currentTrack.url);
          }).catch(err => {
            console.error("[Player.tsx] Audio play prevented:", err);
            console.error("[Player.tsx] Track URL that failed:", currentTrack.url);
            
            // More detailed error logging for debugging
            if (err.name === 'NotSupportedError') {
              console.error("[Player.tsx] Format not supported by browser");
            } else if (err.name === 'NotAllowedError') {
              console.error("[Player.tsx] Playback not allowed without user interaction");
            } else if (err.name === 'AbortError') {
              console.error("[Player.tsx] Playback aborted");
            } else if (err.name === 'NetworkError') {
              console.error("[Player.tsx] Network error occurred while loading audio");
            }
          });
        } else {
          console.warn("[Player.tsx] play() did not return a promise. Browser might not fully support it or audio element is not ready.");
        }
      } else {
        console.log("[Player.tsx] isPlaying is false. Pausing audio if it's playing.");
        if (!audioRef.current.paused) {
          audioRef.current.pause();
          console.log("[Player.tsx] Audio paused.");
        }
      }
    } else {
      console.log("[Player.tsx] No current track. Clearing src and pausing.");
  ...
  useEffect(() => {
    ...
    if (isPlaying && currentTrack) {
      ...
      if (audioRef.current.src !== currentTrack.url) {
        console.log("[Player.tsx] isPlaying effect: src mismatch. Setting src to:", currentTrack.url);
        audioRef.current.src = currentTrack.url;
        audioRef.current.load(); // Explicitly call load
      }
      console.log("[Player.tsx] isPlaying effect: Attempting to play audio. Current src:", audioRef.current.src);
      const playPromise = audioRef.current.play();
      if (playPromise !== undefined) {
        playPromise.then(_ => {
          console.log("[Player.tsx] isPlaying effect: Audio playback started successfully for:", currentTrack.url);
        }).catch(err => {
          console.warn("[Player.tsx] isPlaying effect: Could not play audio:", err);
          console.warn("[Player.tsx] isPlaying effect: Track URL that failed:", currentTrack.url);
        });
      } else {
        console.warn("[Player.tsx] isPlaying effect: play() did not return a promise.");
      }
